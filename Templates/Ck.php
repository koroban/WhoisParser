...
class Template_Ck extends AbstractTemplate
{
    ...
    public function postProcess(&$WhoisParser)
    {
        ...
        if ($ResultSet->changed != '') {
            $ResultSet->changed = preg_replace('/([a-z@\.\x20]*)/', '', $ResultSet->changed);
        }
        
        if ($ResultSet->expires != '') {
            $ResultSet->expires = preg_replace('/([a-z@\.\x20]*)/', '', $ResultSet->expires);
        }
        ...
        foreach ($ResultSet->contacts as $contactType => $contactArray) {
            foreach ($contactArray as $contactObject) {
                ...
                if ($contactObject->changed != '') {
                    $contactObject->changed = preg_replace('/([a-z@\.\x20]*)/', '', $contactObject->changed);
                }
            }
        }
    }
